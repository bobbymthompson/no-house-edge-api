...
beforeEach(async () => {
  user1 = await User.create({ name: 'user', email: 'a@a.com' })
  user2 = await User.create({ name: 'user', email: 'b@b.com' })
  admin = await User.create({ email: 'c@c.com', role: 'admin' })
...
test('POST /users 201 (master)', async () => {
  const { status, body } = await request(app())
    .post('/')
    .send({ access_token: masterKey, email: 'd@d.com', role: 'user' })
  expect(status).toBe(201)
  expect(typeof body).toBe('object')
  expect(body.email).toBe('d@d.com')
})
...
test('POST /users 201 (master)', async () => {
  const { status, body } = await request(app())
    .post('/')
    .send({ access_token: masterKey, email: 'd@d.com', role: 'admin' })
  expect(status).toBe(201)
  expect(typeof body).toBe('object')
  expect(body.email).toBe('d@d.com')
})
...
test('POST /users 400 (master) - invalid email', async () => {
  const { status, body } = await request(app())
    .post('/')
    .send({ access_token: masterKey, email: 'invalid' })
...
test('POST /users 400 (master) - invalid role', async () => {
  const { status, body } = await request(app())
    .post('/')
    .send({ access_token: masterKey, email: 'd@d.com', role: 'invalid' })
  expect(status).toBe(400)
  expect(typeof body).toBe('object')
...
test('POST /users 401 (admin)', async () => {
  const { status } = await request(app())
    .post('/')
    .send({ access_token: adminSession, email: 'd@d.com' })
  expect(status).toBe(401)
})
...
test('POST /users 401 (user)', async () => {
  const { status } = await request(app())
    .post('/')
    .send({ access_token: session1, email: 'd@d.com' })
  expect(status).toBe(401)
})
